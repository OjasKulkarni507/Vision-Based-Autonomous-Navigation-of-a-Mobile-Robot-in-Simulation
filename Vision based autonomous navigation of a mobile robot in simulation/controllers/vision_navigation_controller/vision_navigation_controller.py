"""
================================================================================
  Hybrid Autonomous Navigation Controller for E-puck (Webots)
  + Floor Plane Segmentation (Strategy 3) — OPTIMISED v3
================================================================================

ROOT-CAUSE ANALYSIS: STRIDE BLIND-SPOT  (v2 bugs — already fixed)
───────────────────────────────────────
The original STATE_AVOID_DRIVE (forward stride) phase had two compounding bugs:

  BUG 1 — Detection gating:
    Avoidance-entry (step 5 in the main loop) was guarded by:
        if current_state == STATE_SEEK and center_blocked ...
    This means obstacle detection was COMPLETELY DISABLED during AVOID_DRIVE.
    The robot was mechanically blind for up to STRIDE_DURATION × TIME_STEP ms.
    At FORWARD_SPEED (≈ 0.47 m/s) over 45 steps × 32 ms = 1.44 s, the robot
    could travel ~0.68 m with zero obstacle awareness — enough to fully embed
    itself into a new obstacle.

  BUG 2 — Floor model contamination:
    floor_model.update(bgr) was called every step regardless of state.
    During AVOID_DRIVE the robot drives sideways past an obstacle. The obstacle
    occupies the bottom-strip sample region for several frames. With
    FLOOR_LEARN_ALPHA=0.08, after 10 frames the floor mean has shifted ~55%
    toward the obstacle colour. This corrupts the model and causes the robot to
    "not see" the very obstacle it just passed — and any other obstacle of the
    same colour it encounters next.

v3 ROOT-CAUSE ANALYSIS: OSCILLATION WALL-LOCK (orange wall scenario)
──────────────────────────────────────────────────────────────────────
Observed symptom: robot encounters the orange wall at x≈2.1 and enters an
infinite interrupt loop, oscillating LEFT/RIGHT without ever bypassing the wall.

  BUG 3 — Cooldown expires before turn completes (PRIMARY cause):
    STRIDE_INTERRUPT_COOLDOWN (8 steps) < STRIDE_INTERRUPT_TURN_DURATION (12 steps)
    Timeline per oscillation cycle:
      Step  0: interrupt fires → enter AVOID_TURN for 12 steps, set cooldown=8
      Step  8: cooldown reaches 0  ← still INSIDE the turn!
      Step 12: turn completes → enter AVOID_DRIVE (stride step 0)
      Step 13: center_blocked still True (wall hasn't cleared),
               cooldown==0 → interrupt fires IMMEDIATELY (stride_remaining=49)
    This creates an infinite loop: turn→stride-step-1→interrupt→turn→...
    Evidence from log: every other interrupt fires with stride_remaining=49,
    and consecutive interrupt gaps alternate between exactly 13 and 16-17 steps.

  BUG 4 — No escalation when genuinely stuck (SECONDARY cause):
    When the robot is pressed close to a wide obstacle (orange wall is 0.35 m),
    a single 12-step interrupt-turn (~58°) is insufficient to gain angular
    clearance. Even with BUG 3 fixed, repeated short turns merely slide the
    robot laterally along the wall without ever clearing it.

  BUG 5 — No stall detection / forced escape:
    If the robot's position changes by less than a threshold across many
    consecutive interrupt cycles, there is no mechanism to force a longer
    escape maneuver. The robot drifts millimetre by millimetre indefinitely.

v3 SOLUTION ARCHITECTURE
─────────────────────────
  FIX-A — Cooldown > turn duration (resolves BUG 3):
    STRIDE_INTERRUPT_COOLDOWN is set to STRIDE_INTERRUPT_TURN_DURATION + 4.
    The cooldown now expires 4 steps INTO the stride, guaranteeing that the
    obstacle has had time to exit the center zone before another interrupt
    can fire. This eliminates the step-1 re-fire entirely.

  FIX-B — Consecutive interrupt counter with turn escalation (resolves BUG 4):
    A new counter `interrupt_consecutive` tracks how many interrupts have fired
    without an intervening clean stride. Each interrupt adds ESCALATION_STEP
    extra steps to the turn duration (capped at TURN_DURATION, the full turn).
    This progressively rotates the robot further away from the wall until it
    gains enough angular clearance.
    Reset to 0 on: normal stride completion, successful SEEK entry.

  FIX-C — Position stall detection with forced long escape (resolves BUG 5):
    `stall_anchor` records the robot's (x, y) each time an avoidance begins.
    If a new interrupt fires while the robot has moved less than STALL_RADIUS
    from the anchor, `stall_count` increments. When stall_count reaches
    STALL_TRIGGER, the robot is forced into a full TURN_DURATION turn (ignoring
    the shorter interrupt duration) and stall_count resets. This handles the
    case where BUG 4 escalation alone cannot break the wall-hug pattern.

CHANGE LOG  (search "# FIX" to find every changed line)
──────────────────────────────────────────────────────────
  (v2 fixes — unchanged)
  FIX-1a  New parameter STRIDE_INTERRUPT_COOLDOWN
  FIX-1b  New parameter STRIDE_INTERRUPT_TURN_DURATION
  FIX-2   floor_model.update() gated to STATE_SEEK only
  FIX-3   AVOID_DRIVE block: obstacle check + interrupt logic
  FIX-4   Interrupt direction computed as opposite of current avoid_direction
  FIX-5   New state variable: stride_interrupt_cooldown
  FIX-6   draw_debug extended with stride interrupt indicator
  FIX-7   Console logging for stride interrupt events

  (v3 fixes — new)
  FIX-A   STRIDE_INTERRUPT_COOLDOWN raised to STRIDE_INTERRUPT_TURN_DURATION + 4
  FIX-B   New parameters: INTERRUPT_ESCALATION_STEP, MAX_INTERRUPT_TURN_DURATION
  FIX-C   New parameters: STALL_RADIUS, STALL_TRIGGER
  FIX-D   New state variables: interrupt_consecutive, stall_count, stall_anchor
  FIX-E   Interrupt block: escalated turn duration + stall override
  FIX-F   interrupt_consecutive reset on stride completion and SEEK entry
  FIX-G   stall_anchor update on every avoidance entry from SEEK
  FIX-H   Console logging for escalation and stall events

All original navigation logic (SEEK heading/speed control, turn phase,
cooldown, arrival, floor model) is COMPLETELY UNCHANGED.
"""
